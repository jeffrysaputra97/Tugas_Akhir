...
\noindent Pengolahan citra merupakan suatu proses yang berfokus pada dua hal utama, yaitu meningkatkan kualitas informasi pada citra untuk membantu interpretasi manusia dan pemrosesan data citra untuk nantinya disimpan, ditransmisikan, dan digunakan sebagai referensi pengetahuan dari suatu mesin \textit{autonomous} \cite{gonzalez}.
...
\noindent Karena citra yang digunakan berbentuk persegi panjang maka jarak diagonal memenuhi persamaan \ref{eq:PersamaanJarakDiagonalPersegi} \cite{rectangle}:
...
\begin{table}[H]
	\begin{adjustbox}{width=1\textwidth}
		\begin{tabular}{|p{13.55cm}|}
			...
			\begin{equation}
			D = \sqrt{N^2 + M^2}
			\label{eq:PersamaanJarakDiagonalPersegi}
			\end{equation}\\
			\hline
		\end{tabular}
	\end{adjustbox}
...
\noindent Sehingga rentang nilai \textit{rho} yang digunakan dalam penelitian ini adalah memenuhi aturan seperti ditunjukkan pada pertidaksamaan \ref{eq:RentangRho2}:
...
\noindent Segmentasi merupakan proses pembagian daerah dalam suatu citra untuk dipisahkan ke dalam segmen-segmen tertentu. Tujuan utama dari proses segmentasi adalah menyederhanakan dan/atau mengubah representasi citra menjadi sesuatu yang memiliki arti tertentu dan lebih mudah dianalisis \cite{gonzalez}. Segmentasi citra secara umum digunakan untuk mengenali objek dan batas-batas (garis) dalam citra. 

\noindent Secara garis besar, terdapat tiga cara utama untuk melakukan segmentasi, yaitu dengan cara \textit{Thresholding}, segmentasi tepian, dan segmentasi wilayah. Contoh dari metode segmentasi \textit{Thresholding} adalah \textit{Basic Global Thresholding}, \textit{Otsu}, \textit{Multiple Threshold}, dan \textit{Variable Thresholding}. Contoh metode segmentasi tepian adalah deteksi tepi standar (\textit{Sobel}, \textit{Robert}, \textit{Prewitt}, \textit{Canny}), \textit{Marr-Hildreth edge detector}, \textit{Short Response Hilbert Transform}, dan \textit{Watersheds}. Sedangkan untuk metode segmentasi wilayah contohnya adalah \textit{Region Growing}, \textit{Data Clustering}, \textit{Partitional Clustering}, dan \textit{Cheng-Jin Kuo's Method}.\\
...
\noindent\textit{Histogram of Oriented Gradients} merupakan salah satu teknik pengambilan fitur yang bertujuan untuk mengambil informasi penting dari sebuah citra. Cara kerja metode ini yaitu dengan mengevaluasi histogram lokal yang sudah ternormalisasi secara baik dari distribusi gradien citra dalam \textit{grid} yang padat. Teknik mengekstrak fitur untuk metode ini yaitu dari distribusi lokal dari intensitas gradient tiap piksel yang terdapat pada sebuah objek citra \cite{hog}. Dalam metode \textit{Histogram of Oriented Gradient}, ukuran sel berupa kumpulan atau gabungan piksel dan blok berupa kumpulan atau gabungan sel beserta jumlah \textit{orientation bin} yang merupakan tempat untuk menampung hasil arah dan besar gradien akan mempengaruhi hasil keluaran fitur vektor yang dihasilkan dan juga akurasi yang didapat. Pertama untuk setiap piksel dari citra akan dihitung gradiennya dari sumbu x dan y dengan menggunakan persamaan \ref{eq:PersamaanGradienX} dan \ref{eq:PersamaanGradienY} :
...
\noindent Setelah didapat nilai gradient dari sumbu x dan y untuk setiap pikselnya, proses selanjutnya adalah menghitung besar nilai dan arah gradiennya dengan menggunakan rumus \ref{eq:PersamaanMagnitude} dan \ref{eq:PersamaanArah}:
...
\noindent Kemudian, setiap piksel citra akan dibagi ke dalam beberapa sel yang dari setiap sel, akan dihitung persebaran \textit{Histogram of Oriented Gradient}-nya melalui proses \textit{voting}. Proses \textit{voting} dalam \textit{Histogram of Oriented Gradient} pertama akan menentukan nilai-nilai dari \textit{bin} dengan membagi total jumlah sudut gradien ke dalam jumlah \textit{orientation bin}. Kemudian untuk setiap arah sudut gradien dari setiap piksel dalam sel akan dimasukkan ke dalam rentang \textit{orientation bin} yang sudah ditentukan pada pertama kali, kemudian membagi besar nilai gradiennya dengan \textit{orientation bin} yang terkait. Sebagai contoh, jika jumlah \textit{bin} yang digunakan adalah 4 \textit{bin}. Maka ukuran \textit{range} sudut setiap bin adalah 180/4 = 45 derajat. Ilustrasinya dapat dilihat pada Gambar \ref{fig:IlustrasiBin}.

\begin{adjustbox}{width=1\textwidth}
	\noindent\begin{minipage}{\linewidth}
		\centering\includegraphics[width=6cm]{images/IlustrasiBin.png}
		\captionof{figure}{Ilustrasi \textit{bin} dengan jumlah 4\\}
		\label{fig:IlustrasiBin}
	\end{minipage}
\end{adjustbox}\\

\noindent Setelah \textit{Histogram of Oriented Gradient} sudah dibuat untuk setiap sel, proses selanjutnya adalah melakukan normalisasi terhadap hasil \textit{vote} pada setiap \textit{bin} dalam sel. Normalisasi akan dilakukan dalam 1 blok, dengan ukuran blok merupakan m $\times$ n sel. Terdapat 4 macam metode untuk normalisasi, yaitu: \textit{L2-Norm}, \textit{L2-Hys}, \textit{L1-sqrt}, dan \textit{L1-Norm}. Persamaan \ref{eq:PersamaanL1Norm} sampai \ref{eq:L2-Norm} berikut merupakan persamaan untuk metode normalisasi \textit{L1-Norm}, \textit{L1-Sqrt}, dan \textit{L2-Norm}:
...
\begin{table}[H]
	...
	\begin{adjustbox}{width=1\textwidth}
		\begin{tabular}{|p{13.55cm}|}
			\hline
			\begin{equation} 
			\text{L1-Norm}_{i} = \frac{V_{i}}{\sum_{j=1}^{N}V_{j}}
			\label{eq:PersamaanL1Norm}
			\end{equation}\\
			\hline
		\end{tabular}
	\end{adjustbox}
\end{table}
\noindent Dimana:\\
$V$ = bobot vektor yang merepresentasikan nilai setiap \textit{bin}\\
$j$ = nilai \textit{counter} dari 1 sampai N\\
...
\begin{table}[H]
	...
	\begin{adjustbox}{width=1\textwidth}
		\begin{tabular}{|p{13.55cm}|}
			\hline
			\begin{equation} 
			\text{L1-Sqrt}_{i} = \sqrt{\frac{V_{i}}{\sum_{j=1}^{N}V_{j}}}
			\end{equation}\\
			\hline
		\end{tabular}
	\end{adjustbox}
...
\begin{table}[H]
	...
	\begin{adjustbox}{width=1\textwidth}
		\begin{tabular}{|p{13.55cm}|}
			\hline
			\begin{equation} 
			\text{L2-Norm}_{i} = \frac{V_{i}}{\sqrt{\sum_{j=1}^{N}V_{j}^2}}
			\label{eq:L2-Norm}
			\end{equation}\\
			\hline
		\end{tabular}
...
\noindent Dimana $w^T$ adalah vektor berat dan $x$ adalah vektor masukan dan $b$ merupakan nilai bias. Tanda “.” menggambarkan perkalian dot vektor.
\begin{table}[H]
	\small
	\begin{adjustbox}{width=1\textwidth}
...
\subsection{\textit{Character Recognition Rate}}
\noindent \textit{Character Recognition Rate} atau CRR adalah alat ukur untuk mengetahui keberhasilan pengenalan karakter dari suatu aplikasi pengenalan karakter \cite{gou2016}. Perhitungan persentase CRR dilakukan dengan cara membagi jumlah karakter yang dapat dikenali terhadap jumlah karakter yang terdeteksi oleh sistem seperti yang dapat dilihat pada persamaan \ref{eq:crr}. 
...
\begin{table}[H]
	\begin{adjustbox}{width=1\textwidth}
		\begin{tabular}{|p{13.55cm}|}
			\hline
			\begin{equation}
			CRR = \frac{\text{\textit{Number of Correctly Recognized Characters}}}{\text{\textit{Number of All Detected Characters}}}
			\label{eq:crr}
			\end{equation}\\
			\hline
		\end{tabular}
...
\subsection{\textit{Overall Performance}}
\noindent \textit{Overall Performance} atau OVR adalah alat ukur untuk mengetahui performa keseluruhan dari aplikasi pada penelitian ini \cite{gou2016}. Perhitungan persentase OVR dilakukan dengan cara membagi jumlah plat nomor yang terdeteksi dan dapat dikenali dengan benar terhadap jumlah keseluruhan plat nomor pada \textit{dataset} seperti yang dapat dilihat pada persamaan \ref{eq:ovr}. 
...
\begin{table}[H]
	\begin{adjustbox}{width=1\textwidth}
		\begin{tabular}{|p{13.55cm}|}
			\hline
			\begin{equation}
			OVR = \frac{\text{\textit{Number of Correctly Detected and Recognized LPs}}}{\text{\textit{Number of All Ground Truth LPs}}}
			\label{eq:ovr}
			\end{equation}\\
			\hline
		\end{tabular}
...
\noindent Bagian ini akan menjelaskan \textit{library} yang digunakan dalam penelitian ini. \textit{Library} yang digunakan diantaranya \textit{OpenCV} untuk melakukan tahapan \textit{preprocessing}, \textit{Weka SVM} untuk melakukan tahapan klasifikasi karakter, \textit{JavaOCR} untuk melakukan segmentasi terhadap citra plat. Untuk penjelasan lebih lanjut dari setiap \textit{library} akan disampaikan pada subbab berikut.\\
...
\begin{small}
	\begin{longtable}{| p {0.5cm} | p {6cm} | p {6cm} |}
		...
		\hline
		2 & Imgproc.cvtColor(Mat src, Mat dst, int code) & Mengkonversi jenis warna citra dengan parameter citra RGB, matriks citra tujuan, dan \textit{code}. \textit{Code} digunakan untuk memilih tipe konversi citra tersebut, misal \textit{grayscale}.\\
		\hline
		...
		\hline
		4 & Imgproc.GaussianBlur(Mat src, Mat dst, Size ksize, double sigmaX) & Melakukan \textit{Gaussian Filter} dengan parameter citra \textit{grayscale}, matriks citra tujuan, ukuran \textit{kernel} dan nilai \textit{sigma} yang diberikan.\\
		\hline
		5 & Imgproc.threshold(Mat src, Mat dst, double thresh, double maxval, int type) & Melakukan \textit{thresholding} dengan parameter citra \textit{grayscale}, matriks citra tujuan, nilai \textit{threshold}, nilai maksimum, serta jenis metode \textit{thresholding} yang digunakan, misalnya metode \textit{thresholding} Otsu. Keluaran dari \textit{function} ini adalah citra biner.\\
		\hline
		6 & Imgproc.findContours(Mat image, List$<$MatOfPoint$>$ contours, Mat hierarchy, int mode, int method) & Melakukan pencarian kontur terhadap citra biner yang dijadikan masukkan.\\
		\hline
		7 & Imgproc.contourArea(Mat contour) & Melakukan perhitungan luas area untuk setiap kontur dari hasil pencarian kontur menggunakan \textit{function} \textit{Imgproc.findContours()}. \\
		\hline
		...
		\hline
		9 & Imgproc.HoughLines(Mat image, double minTheta, double maxTheta, double rho, Mat peak) & Mengekstraksi segmen garis berdasarkan \textit{peaks}. \textit{Peaks} adalah matriks hasil keluaran \textit{function} \textit{houghpeaks} yang berisi koordinat baris dan kolom dari \textit{hough transform bins} yang digunakan untuk melakukan pencarian segmen garis.\\
		\hline
	\end{longtable}
\end{small}
\subsubsection{Weka}
\noindent Weka adalah kumpulan dari algoritme pembelajaran mesin yang digunakan untuk menyelesaikan permasalahan \textit{data mining}. \textit{Library} ini berbasis bahasa pemrograman Java dan dapat berjalan di hampir seluruh \textit{platform}. Dalam penelitian ini, \textit{library} Weka digunakan untuk melakukan proses klasifikasi menggunakan metode \textit{Support Vector Machine}.
\begin{small}
	\begin{longtable}{| p {0.5cm} | p {6cm} | p {6cm} |}
		\caption{Tabel fungsi \textit{Library} Weka} 
		\label{tbl:FunctionWeka}\\
		\hline
		\textbf{No} & \textbf{\textit{Function}} & \textbf{Deskripsi}\\
		\hline
		\endfirsthead
		...
		\textbf{No} & \textbf{\textit{Function}} & \textbf{Deskripsi}\\
		\endhead
		1 & libsvm.setOptions(String[] options) & Mengkonfigurasi dan menginisialisasi parameter dari \textit{classifier} yang akan digunakan.\\
		\hline
		2 & libsvm.buildClassifier(Instances insts) & Membangun \textit{classifier} dari dataset yang diberikan sesuai dengan konfigurasi dan parameter yang digunakan.\\
		\hline
...
\begin{small}
	\begin{longtable}{| p {0.5cm} | p {6cm} | p {6cm} |}
		...
		\label{tbl:FunctionJavaOCR}\\
		\hline
		\textbf{No} & \textbf{\textit{Function}} & \textbf{Deskripsi}\\
		\hline
		\endfirsthead
		
		\multicolumn{3}{c}{\textbf{\tablename~\thetable} \textit{Tabel fungsi \textit{Library} JavaOCR} (Lanjutan)}\\
		\hline
		\textbf{No} & \textbf{\textit{Function}} & \textbf{Deskripsi}\\
		\endhead
...
\noindent Pada referensi pertama \cite{nugroho} menggunakan metode pengolahan citra untuk mencari kandidat-kandidat pita dengan menghitung histogram citra untuk mendapatkan lokasi plat nomor kendaraan, kemudian setiap karakter dari plat nomor yang didapatkan disegmentasi dengan cara menghitung grafik horizontal citra, kemudian untuk metode klasifikasi karakter yang digunakan adalah \textit{Learning Vector Quantization}.
...
\noindent Pada bagian ini akan diulas mengenai objek penelitian yaitu plat nomor kendaraan dan hal-hal yang berkaitan seperti spesifikasi plat nomor kendaraan Indonesia, jenis plat nomor kendaraan bermotor di Indonesia, dan \textit{dataset Tel-U Vehicle Data-set V1.0} yang akan digunakan dalam penelitian.\\
